...
hbb_name = 'CuteKVM' + ('.exe' if windows else '')
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/CuteKVM.exe')
    os.chdir('../..')
    if os.path.exists('./CuteKVM.exe'):
        os.replace('./target/release/CuteKVM-portable-packer.exe',
                   './CuteKVM.exe')
    else:
        os.rename('./target/release/CuteKVM-portable-packer.exe',
                  './CuteKVM.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/CuteKVM.exe')
    os.rename('./CuteKVM.exe', f'./CuteKVM-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/CuteKVM-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        system2('mv target/release/CuteKVM.exe target/release/CuteKVM.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\CuteKVM.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/CuteKVM.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/CuteKVM-{version}-win7-install.exe')
        system2('mv ../../{res_dir}/CuteKVM-{version}-win7-install.exe ../..')
